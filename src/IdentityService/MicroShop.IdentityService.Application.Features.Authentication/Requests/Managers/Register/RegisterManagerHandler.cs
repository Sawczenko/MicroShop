...
internal sealed class RegisterManagerHandler : ManagerHandlerBase<RegisterManager, RegisterResponse>
{
    public RegisterManagerHandler(IManagerContainer container) 
        : base(container) { }

    public override async Task<RequestResult<RegisterResponse>> Handle(RegisterManager manager, CancellationToken cancellationToken)
    {
        var existingUser = await Send(new GetUserByNameQuery(manager.Request.UserName), cancellationToken);

        var userExistsResult = await Validate(new UserExistsValidator(existingUser, manager.Request.UserName), cancellationToken);
        ...
        return RequestResult<RegisterResponse>.Success(new RegisterResponse
        {
